...
;
; PGirvan 19/03/2023
; Adapted to work with TIFF files rather than .pma using MultiSplit

pro mapping_maker, run, text_ID
	...
	if N_PARAMS() eq 0 then begin
		run = DIALOG_PICKFILE(PATH='c:\user\tir', TITLE='Select a File to make a mapping file', /READ, FILTER = '*.tif')
	...
	frame_average_ch1 = frame_average[0:303,0:303]
	frame_average_ch1 = 255*(frame_average_ch1-min(frame_average_ch1))/(max(frame_average_ch1)-min(frame_average_ch1))	
	frame_average_ch2 = frame_average[304:607,0:303]
	frame_average_ch2 = 255*(frame_average_ch2-min(frame_average_ch2))/(max(frame_average_ch2)-min(frame_average_ch2))	
	frame_average_ch3 = frame_average[0:303,304:607]
	frame_average_ch3 = 255*(frame_average_ch3-min(frame_average_ch3))/(max(frame_average_ch3)-min(frame_average_ch3))
	frame_average_ch4 = frame_average[304:607,304:607]
	frame_average_ch4 = 255*(frame_average_ch4-min(frame_average_ch4))/(max(frame_average_ch4)-min(frame_average_ch4))
	frame_norm = [[frame_average_ch1,frame_average_ch2],[frame_average_ch3,frame_average_ch4]]
	frame_average = 255*(frame_average-min(frame_average))/(max(frame_average)-min(frame_average))
	...
	WRITE_TIFF, run + "_aveNorm.tif", frame_norm, 1, RED = R_ORIG, GREEN = G_ORIG, BLUE = B_ORIG, compression=2	

	WIDGET_CONTROL, text_ID, SET_VALUE=("Intensity Median value for " + run + ".tif :" + STRING(median(frame))), /APPEND, /SHOW
	...
	temp = smooth(frame_norm, 2, /EDGE_TRUNCATE)
...
;	modified_frame = frame - byte(mininum_intensity_matrix)
	modified_frame = frame_norm - mininum_intensity_matrix


	; thresholds the image for peak finding purposes

	medianofFrame_left = float(median(modified_frame(10:290, 10:290)))
	varianceofFrame_left = variance(modified_frame(10:290, 10:290))
	medianofFrame_middle = float(median(modified_frame(310:600, 10:290)))
	varianceofFrame_middle = variance(modified_frame(310:600, 10:290))
	medianofFrame_right = float(median(modified_frame(10:290, 310:600)))
	varianceofFrame_right = variance(modified_frame(10:290, 310:600))
	...
	window, 2, xsize=film_width, ysize=film_height
	...
	for j = 10, film_height - 11 do begin
		if j eq 294 then j = 314 ;skip region where Ch1-Ch2 overlab with Ch3-Ch4
		for i = 10, film_width - 11 do begin
			if i eq 294 then i = 314   ; skip region where Ch1-Ch3 overlab with Ch2-Ch4
	...
	for j = 0, 2 do begin
		...
		case !MOUSE.BUTTON of
			1 : begin
					if point_x lt 304 and point_y lt 304 then begin
						x_left(j) = point_x
						y_left(j) = point_y
						x_middle(j) = point_x + 304
						y_middle(j) = point_y
						x_right(j) = point_x
						y_right(j) = point_y + 304
					endif else begin
					if point_x gt 304 and point_y lt 304 then begin
						if x_left(j) eq 0 and x_right(j) eq 0 then begin
							x_left(j) = point_x - 304
							y_left(j) = point_y
							x_middle(j) = point_x
							y_middle(j) = point_y
							x_right(j) = point_x - 304
							y_right(j) = point_y + 304
						endif else begin
							x_middle(j) = point_x
							y_middle(j) = point_y
					...
					if point_x lt 304 and point_y gt 304 then begin
                		if x_left(j) eq 0 and x_middle(j) eq 0 then begin
                  			x_left(j) = point_x 
							y_left(j) = point_y - 304
							x_middle(j) = point_x + 304
							y_middle(j) = point_y - 304
							x_right(j) = point_x
							y_right(j) = point_y
						endif else begin
                    		x_right(j) = point_x
	...
	for i = 0, NumberofGoodLocations - 1 do begin
		if GoodLocations_x(i) lt 304 then begin
					...
					for k = 0, NumberofGoodLocations - 1 do begin
						if(abs(GoodLocations_x(k) - x_right_expect) lt 3) and (abs(GoodLocations_y(k) - y_right_expect) lt 3) then begin
              			...
              			xx_left(numberofPairs) = GoodLocations_x(i)
							...
							xx_middle(numberofPairs) = GoodLocations_x(j) - 304
							yy_middle(numberofPairs) = GoodLocations_y(j)
							xx_right(numberofPairs) = GoodLocations_x(k)
							yy_right(numberofPairs) = GoodLocations_y(k) - 304
							numberofPairs++
						endif
					endfor
				endif
	...
	;block for writing out images to assess mapping values
	;the various channels are
	;frame_average_ch1
	;frame_average_ch2
	;frame_average_ch3
	
	frame_average_ch2_warped = POLY_2D(frame_average_ch2, P12, Q12, 2)
	frame_average_ch3_warped = POLY_2D(frame_average_ch3, P13, Q13, 2)
	WRITE_TIFF, run + "_ch1.tif", frame_average_ch1, 1, RED = R_ORIG, GREEN = G_ORIG, BLUE = B_ORIG, compression=2
	WRITE_TIFF, run + "_ch2.tif", frame_average_ch2, 1, RED = R_ORIG, GREEN = G_ORIG, BLUE = B_ORIG, compression=2
	WRITE_TIFF, run + "_ch3.tif", frame_average_ch3, 1, RED = R_ORIG, GREEN = G_ORIG, BLUE = B_ORIG, compression=2
	WRITE_TIFF, run + "warp_ch1.tif", frame_average_ch1, 1, RED = R_ORIG, GREEN = G_ORIG, BLUE = B_ORIG, compression=2
	WRITE_TIFF, run + "warp_ch2.tif", frame_average_ch2_warped, 1, RED = R_ORIG, GREEN = G_ORIG, BLUE = B_ORIG, compression=2
	WRITE_TIFF, run + "warp_ch3.tif", frame_average_ch3_warped, 1, RED = R_ORIG, GREEN = G_ORIG, BLUE = B_ORIG, compression=2
  ;end of block for writing out mapped images
